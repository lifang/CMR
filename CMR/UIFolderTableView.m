...
-(id<UIFolderTableViewDelegate>)folderDelegate {
    return (id<UIFolderTableViewDelegate>)[super delegate];
}
-(void)setFolderDelegate:(id<UIFolderTableViewDelegate>)folderDelegate {
    _folderDelegate = folderDelegate;
    _flags.delegateWillReloadData = [_folderDelegate respondsToSelector:@selector(tableViewWillReloadData:)];
    _flags.delegateDidReloadData = [_folderDelegate respondsToSelector:@selector(tableViewDidReloadData:)];
}
...
- (void)reloadData {
    ...
    if (_flags.reloading == NO) {
        ...
        if (_flags.delegateWillReloadData) {
            [(id<UIFolderTableViewDelegate>)_folderDelegate tableViewWillReloadData:self];
        }
...
- (void)finishReload {
    ...
    if (_flags.delegateDidReloadData) {
        [(id<UIFolderTableViewDelegate>)_folderDelegate tableViewDidReloadData:self];
    }
}
...
- (FolderCoverView *)buttonForRect:(CGRect)aRect
    ...
    if (isTop) {
        UITableViewCell *cell = [self cellForRowAtIndexPath:indexPath];
        button = [[[FolderCoverView alloc] initWithFrame:aRect offset:cell.frame.size.height] autorelease];
