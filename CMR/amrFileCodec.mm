...
int DecodeAMRFileToWAVEFile(const char* pchAMRFileName, const char* pchWAVEFilename)
{
    ...
    NSString *fromStr =[[NSString alloc]initWithCString:pchAMRFileName encoding:0];
    NSString *toStr =[[NSString alloc]initWithCString:pchWAVEFilename encoding:0];
    
    NSString *docDirPath = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString *filePath = [NSString stringWithFormat:@"%@/%@", docDirPath ,fromStr];
    
	fpamr = fopen([filePath cStringUsingEncoding:NSASCIIStringEncoding], "rb");
	...
	// 创建并初始化WAVE文件
    NSString *filePath2 = [NSString stringWithFormat:@"%@/%@", docDirPath ,toStr];
	
	fpwave = fopen([filePath2 cStringUsingEncoding:NSASCIIStringEncoding], "wb");
	...
	// 重写WAVE文件头
	fpwave = fopen([filePath2 cStringUsingEncoding:NSASCIIStringEncoding], "r+");
	WriteWAVEFileHeader(fpwave, nFrameCount);
	fclose(fpwave);
